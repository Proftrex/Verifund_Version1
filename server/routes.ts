...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get pending invites
  app.get('/api/admin/invites/pending', isAuthenticated, async (req: any, res) => {
    try {
      const adminUserId = req.user?.claims?.sub || req.user?.sub;
      const adminUser = await storage.getUser(adminUserId);
  ...
  app.get('/api/admin/invites/accepted', isAuthenticated, async (req: any, res) => {
    try {
      const adminUserId = req.user?.claims?.sub || req.user?.sub;
      const adminUser = await storage.getUser(adminUserId);
      ...
      const acceptedInvites = await storage.getAcceptedInvites();
      res.json(acceptedInvites);
    } catch (error) {
      console.error("Error fetching accepted invites:", error);
      res.status(500).json({ message: "Failed to fetch accepted invites" });
    }
  });

  // Get declined invites
  app.get('/api/admin/invites/declined', isAuthenticated, async (req: any, res) => {
    try {
      const adminUserId = req.user?.claims?.sub || req.user?.sub;
      const adminUser = await storage.getUser(adminUserId);
      ...
      const declinedInvites = await storage.getDeclinedInvites();
      res.json(declinedInvites);
    } catch (error) {
      console.error("Error fetching declined invites:", error);
      res.status(500).json({ message: "Failed to fetch declined invites" });
    }
  });

  // Get inactive admins (admin, manager, support)
  app.get('/api/admin/admins/inactive', isAuthenticated, async (req: any, res) => {
    try {
      const adminUserId = req.user?.claims?.sub || req.user?.sub;
      const adminUser = await storage.getUser(adminUserId);
      ...
      const inactiveAdmins = await storage.getInactiveAdmins();
      res.json(inactiveAdmins);
    } catch (error) {
      console.error("Error fetching inactive admins:", error);
      res.status(500).json({ message: "Failed to fetch inactive admins" });
    }
  });

  // Get active admins (admin, manager, support)
  app.get('/api/admin/admins/active', isAuthenticated, async (req: any, res) => {
    try {
      const adminUserId = req.user?.claims?.sub || req.user?.sub;
      const adminUser = await storage.getUser(adminUserId);
  ...
  // Get inactive admin staff
  app.get('/api/admin/staff/inactive', isAuthenticated, async (req: any, res) => {
    try {
      const adminUserId = req.user?.claims?.sub || req.user?.sub;
      const adminUser = await storage.getUser(adminUserId);
      ...
      const inactiveStaff = await storage.getInactiveAdminStaff();
      res.json(inactiveStaff);
    } catch (error) {
      console.error("Error fetching inactive admin staff:", error);
      res.status(500).json({ message: "Failed to fetch inactive admin staff" });
    }
  });

  // Get active admin staff
  app.get('/api/admin/staff/active', isAuthenticated, async (req: any, res) => {
    try {
      const adminUserId = req.user?.claims?.sub || req.user?.sub;
      const adminUser = await storage.getUser(adminUserId);
      ...
      const activeStaff = await storage.getActiveAdminStaff();
      res.json(activeStaff);
    } catch (error) {
      console.error("Error fetching active admin staff:", error);
      res.status(500).json({ message: "Failed to fetch active admin staff" });
    }
  });
    ...
    try {
      ...
      if (attachments) {
        ...
        if (Array.isArray(parsedAttachments) && parsedAttachments.length > 0) {
          // For now, just skip ObjectUploader attachments since they're not working
          // The form should use standard file upload instead
          console.log('📎 Skipping ObjectUploader attachments - use standard file upload instead');
        }
      }
