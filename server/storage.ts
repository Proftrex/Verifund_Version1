  ...
  async getCampaignReports(): Promise<any[]> {
    try {
      ...
      const campaignReports = allFraudReports.filter((report: any) => {
        ...
        // Only include if it's a campaign type AND not creator-related in any way
        return isCampaignType && !isCreatorRelated && !isAutoGenerated && !hasCreatorInReportType && !hasCreatorInDescription;
      });
