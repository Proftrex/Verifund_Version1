...
export class DatabaseStorage implements IStorage {
  ...
  async getCampaignReports(): Promise<any[]> {
    try {
      ...
      console.log('🔍 Total fraud reports:', allFraudReports.length);
      console.log('📊 Sample reports for debugging:', allFraudReports.slice(0, 3).map(r => ({
        id: r.id,
      ...
      const campaignReports = allFraudReports.filter((report: any) => {
        // Only include reports that are specifically about campaigns
        // Exclude any reports that are related to creators (auto-generated or manual)
        ...
        if (isCampaignType || isCreatorRelated) {
          console.log(`📋 Report ${report.id}:`, {
            relatedType: report.relatedType,
            reportType: report.reportType,
            isAutoGenerated,
            hasCreatorInReportType,
            description: report.description?.substring(0, 50) + '...'
          });
        }
        
        // Only include if it's a campaign type AND not creator-related
        const shouldInclude = isCampaignType && !isCreatorRelated && !isAutoGenerated && !hasCreatorInReportType;
        
        if (!shouldInclude && (isCampaignType || isCreatorRelated)) {
          console.log(`❌ Excluding report ${report.id}: Campaign=${isCampaignType}, Creator=${isCreatorRelated}, Auto=${isAutoGenerated}, CreatorType=${hasCreatorInReportType}`);
        }
        
        return shouldInclude;
      });

      console.log('✅ Filtered campaign reports count:', campaignReports.length);
